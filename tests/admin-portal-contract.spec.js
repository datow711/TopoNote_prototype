...
test('legacy root and admin portal keep separate login surfaces', async () => {
  ...
  expect(adminIndex).toContain('id="email-bootstrap-btn"');
});
...
test('admin portal includes its own static assets and role gate', async () => {
  ...
  expect(adminMain).toContain("user?.role === 'admin'");
  expect(adminMain).toContain("user?.role === 'audio_assessor'");
  expect(adminMain).toContain('\u6b64\u5165\u53e3\u50c5\u4f9b\u5be9\u807d\u54e1\u8207\u7ba1\u7406\u54e1\u4f7f\u7528');
});
