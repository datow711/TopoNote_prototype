-- Correct the qualified case id lookup in the assessment history RPC.
-- The original function's return column "id" made an unqualified
-- "where id = ..." ambiguous in PL/pgSQL.
...
begin
  ...
  select ac.* into v_case
  from public.annotation_cases as ac
