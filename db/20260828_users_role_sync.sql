-- Allow the spreadsheet-bound Users sync to create or update review workers.
-- Admin accounts remain outside this general-purpose sync path.
...
begin
  ...
  for v_user in select value from jsonb_array_elements(p_users) loop
    ...
    if v_role = '' then v_role := 'user'; end if;

    if coalesce(v_email, '') = '' or coalesce(v_name, '') = '' then
      continue;
    end if;

    if v_role not in ('user', 'proofreader', 'audio_assessor') then
      continue;
    end if;
    ...
    insert into public.investigators (
      ...
      gen_random_uuid(), v_email, '', v_name, v_role, v_email, v_active,
