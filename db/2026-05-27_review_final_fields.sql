...
add column if not exists final_fields jsonb not null default '{}'::jsonb;

drop view if exists public.app_sheet_sync_queue;
drop function if exists public.approve_task_language(integer, text, text);
drop function if exists public.approve_task_language(integer, text, text, jsonb);

create function public.approve_task_language(
  p_task_id integer,
  p_language text,
  p_reviewed_by text,
...
begin
  ...
  set app_state = '已完成標注',
      final_fields = coalesce(p_fields, '{}'::jsonb),
...
grant execute on function public.approve_task_language(integer, text, text, jsonb) to anon, authenticated;

create view public.app_sheet_sync_queue as
