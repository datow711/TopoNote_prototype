...
- Batch H2 wraps `set_investigator_active` behind root GAS and deploys root GAS Web App version 22.
- Batch H2 one-time Apps Script authorization is complete, and live fake-password smoke testing now reaches the new handler and fails at admin password verification as expected.
- Batch H2-final revoked direct public Supabase execute on `set_investigator_active`; `anon` and `authenticated` can no longer execute it, while `service_role` can.
- Batch H3a implementation wraps `delete_investigator_user` behind root GAS and deploys root GAS Web App version 23. Supabase grants were not changed in H3a.
- Batch H3a-final revoked direct public Supabase execute on `delete_investigator_user`; `anon` and `authenticated` can no longer execute it, while `service_role` can.
...
| Include frontend code | Done for audit | Frontend entrypoints, Supabase calls, and root GAS actions covered in inventory; H2 removes the direct frontend call to `set_investigator_active`; H3a removes the direct frontend call to `delete_investigator_user` | Further frontend cleanup still needs approval. |
...
| Adjust gradually after permission | Started | Batch B + D interim and Batch C applied and verified 2026-06-25; Batch F pushed/deployed/smoke-tested; Batch E documentation correction applied; H2 deployed to root GAS version 22, fake-password smoke-tested after authorization, and H2-final grants applied; H3a deployed to root GAS version 23, fake-password smoke-tested, and H3a-final grants applied | Further batches still need explicit approval. |
...
| H2 implementation | User-approved implementation branch | Frontend calls root GAS `setInvestigatorActive`; root GAS verifies admin password and calls Supabase with service role | Deployed to root GAS Web App `@22`; Apps Script scope authorized; fake-password smoke test passed; merged/pushed by user | Revert frontend/GAS changes or deploy previous GAS version if regression is proven. |
| H2-final grant revoke | User-approved live SQL follow-up | Revoke direct public execute on `set_investigator_active`; keep `service_role` execute | Applied 2026-06-26; `anon=false`, `authenticated=false`, `service_role=true`; root GAS smoke test still reaches admin password verification | Re-grant execute to `anon`/`authenticated` only if a hidden direct caller is proven. |
| H3a implementation | User-approved implementation branch | Frontend calls root GAS `deleteInvestigatorUser`; root GAS verifies admin password and calls Supabase with service role | Deployed to root GAS Web App `@23`; fake-password smoke test passed; merged/pushed by user | Revert frontend/GAS changes or deploy previous GAS version if regression is proven. |
| H3a-final grant revoke | User-approved live SQL follow-up | Revoke direct public execute on `delete_investigator_user`; keep `service_role` execute | Applied 2026-06-26; `anon=false`, `authenticated=false`, `service_role=true`; root GAS smoke test still reaches admin password verification | Re-grant execute to `anon`/`authenticated` only if a hidden direct caller is proven. |
