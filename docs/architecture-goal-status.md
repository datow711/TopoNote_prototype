...
| Include frontend code | Done for audit | Frontend entrypoints, Supabase calls, and root GAS actions covered in inventory; H2 implementation branch removes the direct frontend call to `set_investigator_active`; live fake-password smoke test passed after Apps Script authorization | H2 still needs manual app validation before merge. |
