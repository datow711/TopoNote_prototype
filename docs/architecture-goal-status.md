...
| Include frontend code | Done for audit | Frontend entrypoints, Supabase calls, and root GAS actions covered in inventory; H2 removes the direct frontend call to `set_investigator_active`; H3a removes the direct frontend call to `delete_investigator_user` | H3a still needs manual app validation before merge. |
...
| Adjust gradually after permission | Started | Batch B + D interim and Batch C applied and verified 2026-06-25; Batch F pushed/deployed/smoke-tested; Batch E documentation correction applied; H2 deployed to root GAS version 22, fake-password smoke-tested after authorization, and H2-final grants applied; H3a deployed to root GAS version 23 and fake-password smoke-tested | H3a still needs manual app validation before merge; further batches still need explicit approval. |
...
| H3a implementation | User-approved implementation branch | Frontend calls root GAS `deleteInvestigatorUser`; root GAS verifies admin password and calls Supabase with service role | Deployed to root GAS Web App `@23`; fake-password smoke test passed; manual app delete validation pending | Revert frontend/GAS changes or deploy previous GAS version if regression is proven. |
