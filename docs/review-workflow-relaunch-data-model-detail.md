...
以下是邏輯優先順序。本表已依 **D-001**（未指派留白）與 **D-004**（狀態機收斂為 6 值）更新，對應
`db/20260806_recording_annotation_state.sql` 與 `review-workflow-core.js` 的 `deriveCaseState()`：

| 條件 | `main_state` |
|---|---|
| workflow 未分類 | 主狀態留白 |
| 已選工作流、尚未指派 | 主狀態留白，`assignment_status=未指派`；`annotation_cases.state` 為 `待指派` |
| 書面流程已指派且尚未送校對 | 書面標注中 |
| 音檔流程已指派，仍有音檔未判定 | 錄音中 |
| 音檔流程已指派，所有 active 音檔皆已判定，尚未送校對 | 錄音標注中 |
| 標音版本已送出 | 待校對 |
| 校對者已開始處理 | 校對中 |
| 校對退回標音 | 回到 錄音標注中（音檔流程）或 書面標注中（書面流程） |
| 校對退回音檔 | 回到 錄音中 |
| 校對核准 | 已完成；另由 writeback status 表示是否回寫成功 |

已移除的狀態與理由：

- `待審聽`：音檔審聽進度改由 `app_review_workflow_queue.audio_review_state`（`未審聽`／`待追問`／`已判定`）與各項計數呈現，不佔用主狀態。
- `退回助理處理`：退回直接回到前一個工作階段，退回事實與原因保存在 `proofing_events`。

「錄音標注中」的判定條件刻意**不看**判定結果：`可用`／`不可用`／`待追問` 的分布屬附帶資料，不擴張主狀態。
